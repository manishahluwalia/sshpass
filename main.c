...
static void show_help()
{
    printf("Usage: " PACKAGE_NAME " [-f|-d|-p|-e] [-hV] command parameters\n"
        ...
        "   -P prompt     The prompt used by the ssh host (etc.)\n"
        "                 to ask for password (default \"assword:\")\n"
...
#define VIRGIN_PWTYPE if( args.pwtype!=PWT_STDIN ) { \
    ...
    while( (opt=getopt(argc, argv, "+f:d:p:heVP:"))!=-1 && error==-1 ) {
	switch( opt ) {
	case 'f':
...
int handleoutput( int fd )
{
    ...
    char buffer[strlen(prompt)*100]; // Allow for arbitrary prompts, plenty of junk around the prompt, and reduce the probablity of the prompt falling on a buffer boundary since we do simplistic match()ing
    int ret=0;

    int numread=read(fd, buffer, sizeof(buffer) );

    state1=match( prompt, buffer, numread, state1 );
